...
def run_protocol_agent_v2(question: str) -> str:
    ...
    citations = []
    seen = {}
    def cite(paper: str | None) -> str:
        if not paper:
            return ""
        if paper not in seen:
            seen[paper] = len(citations) + 1
    ...
    prompt = PROMPT_TEMPLATE.format(
        ...
        parameter_edges="\n".join(
            f"- {edge['metadata']['relation']}: {edge['metadata']['value']} (from {cite(edge['metadata']['paper'])} {edge['metadata']['paper']})"
            for edge in edges[:20]
        ),
        instrument_evidence="\n".join(
            f"- **{ev['instrument']}** ({ev['relation']}: {ev['value']}) – {cite(ev['pdf_file'])} {ev['sentence']}" for ev in instrument_evidence
        ),
    )
    answer = local_llm.generate(prompt)
    if citations:
        answer = answer.strip() + "\n\nSources:\n" + "\n".join(f"[{i+1}] {paper}" for i, paper in enumerate(citations))
