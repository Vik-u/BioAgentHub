#!/usr/bin/env python3
"""RAG agent over a topic-specific retrieval stack (optional RL policy support)."""
...
def render_helpful_answer(
    ...
    show_status = os.environ.get("QA_SHOW_STATUS", "1") == "1"
    ...
    if not unverified:
        unverified = [""]
    include_gaps = os.environ.get("QA_INCLUDE_GAPS", "1") == "1"
    if include_gaps:
        lines.append("")
