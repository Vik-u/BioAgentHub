...
TEMP_RANGE_PATTERN = re.compile(
    r"(-?\d+\.?\d*)\s*(?:°\s?C|ºC|degC)?\s*(?:to|-|–)\s*(-?\d+\.?\d*)\s*(?:°\s?C|ºC|degC)",
    re.I,
)
...
def main() -> None:
    ...
    parser.add_argument("--workspace", type=Path, help="Workspace root (sets defaults).")
    parser.add_argument("--method-dir", type=Path, help="Directory with methodology_full JSON files.")
    parser.add_argument("--out-edges", type=Path, help="Output JSONL path for methodology KG edges.")
    parser.add_argument("--summary", type=Path, help="Output JSON path for relation counts.")
    args = parser.parse_args()

    if args.workspace:
        base = args.workspace.resolve()
        args.method_dir = base / "methodology_full"
        args.out_edges = base / "methodology_kg_edges.jsonl"
        args.summary = base / "methodology_kg_summary.json"

    if not args.method_dir or not args.out_edges or not args.summary:
        raise SystemExit("Provide --workspace or all of --method-dir/--out-edges/--summary.")
