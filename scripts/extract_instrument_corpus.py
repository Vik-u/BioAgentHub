#!/usr/bin/env python3
"""Extract full-text corpus from all instrument manuals (recursive)."""
...
def save_page_image(pdf_path: Path, page_number: int, out_dir: Path, scale: float = 1.5) -> Path:
...
def main() -> None:
    ...
    parser.add_argument("--docs-dir", type=Path, default=Path("08_Instrument_Docs"))
    parser.add_argument("--out-dir", type=Path, default=Path("InstrumentGraph"))
    ...
    corpus_index = []
    for pdf_path in iter_pdfs(args.docs_dir):
        text, page_count, methods = extract_pdf(pdf_path, args.export_images, image_dir)
        rel = pdf_path.relative_to(args.docs_dir)
